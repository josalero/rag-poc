...
public final class CandidateNameExtractor {

    private static final int CANDIDATE_LINE_SCAN_LIMIT = 60;
    ...
    private static final Set<String> NAME_BLOCKLIST = Set.of(
            ...
            "personal", "information", "pdf", "disponibilidad", "availability", "immediate",
            "inmediata", "inmediato"
    );
    ...
    private static final Set<String> NAME_ROLE_NOISE = Set.of(
            ...
            "arquitecto", "arquitecta", "gerente", "lider", "líder"
    );
    private static final Set<String> NAME_TECH_NOISE = Set.of(
            ...
            "spring", "boot", "react", "node", "nodejs", "golang", "go",
    ...
    private static final Set<String> SECTION_HEADER_HINTS = Set.of(
            "summary", "profile", "experience", "education", "objective", "contact",
            "skills", "skill", "technical", "tech", "competencies", "competency",
            "certifications", "projects", "highlights", "languages", "knowledge",
            "disponibilidad", "inmediata", "inmediato", "availability", "immediate"
    );
    private static final Set<String> SECTION_HEADER_PREFIX_HINTS = Set.of(
            "skill", "skil", "tecnic", "technic", "experien", "educat", "summar", "profil", "certif",
            "disponib", "inmediat", "availab", "immediat"
    );
    ...
    private static final Pattern LABEL_VALUE_PATTERN = Pattern.compile("^([\\p{L}][\\p{L}\\s/.-]{1,40})\\s*:\\s*(.+)$");
    ...
    public static String extractDisplayName(String filename, String text) {
        List<String> candidateLines = Arrays.stream(text.split("\\n"))
                .map(String::trim)
                .filter(line -> !line.isBlank())
                .limit(CANDIDATE_LINE_SCAN_LIMIT)
                .toList();
    ...
    private static List<String> expandAdjacentNameTokenVariants(List<String> lines, int index) {
        ...
        if (!isSingleTokenNameLine(current)) {
            return List.of();
        }

        List<String> variants = new ArrayList<>();
        if (index + 1 < lines.size() && isSingleTokenNameLine(lines.get(index + 1))) {
            variants.add(current + " " + lines.get(index + 1));
            if (index + 2 < lines.size() && isSingleTokenNameLine(lines.get(index + 2))) {
    ...
    private static boolean isSingleTokenNameLine(String value) {
        ...
        if (NAME_BLOCKLIST.contains(lower)
                ...
                || isSectionHeaderNoiseToken(lower)) {
            return false;
        }
        return hasNameLikeCase(token);
    }

    private static boolean hasNameLikeCase(String token) {
    ...
    public static boolean isPlausibleDisplayName(String value) {
        if (value == null || value.isBlank()) {
            return false;
        }
    ...
    public static boolean looksLikeSkillOrRolePhrase(String value, List<String> knownSkills) {
        if (value == null || value.isBlank()) {
            return false;
        }
    ...
    static String normalizeCandidateName(String line) {
        ...
        if (looksLikeSectionHeaderNoiseLine(compact)) {
            return "";
        }
        ...
        long sectionHeaderNoiseHits = lineWords.stream().filter(CandidateNameExtractor::isSectionHeaderNoiseToken).count();
        if (sectionHeaderNoiseHits >= Math.max(1, lineWords.size() - 1)) {
            return "";
        }
        ...
        if (techNoiseHits >= 1 && lineWords.size() <= 3) {
            return "";
        }
    ...
    private static List<String> expandCandidateLineVariants(String line) {
        ...
        Matcher labeledValueMatcher = LABEL_VALUE_PATTERN.matcher(trimmed);
        if (labeledValueMatcher.matches()) {
            String label = labeledValueMatcher.group(1).trim();
            String value = labeledValueMatcher.group(2).trim();
            if (isLikelyNameLabel(label) && !value.isBlank()) {
                variants.add(value);
            }
        }
    ...
    private static boolean isLikelyNameLabel(String label) {
        if (label == null || label.isBlank()) {
            return false;
        }
        String normalized = normalizeAsciiLetters(label);
        if (normalized.isBlank()) {
            return false;
        }
        if ("name".equals(normalized)
                || "nombre".equals(normalized)
                || "candidate".equals(normalized)
                || "candidateprofile".equals(normalized)
                || "perfilcandidato".equals(normalized)
                || "perfildelcandidato".equals(normalized)) {
            return true;
        }
        return normalized.startsWith("candidateprofile");
    }
    ...
    private static double scoreNameCandidate(String name, int lineIndex) {
        ...
        long sectionHeaderNoiseHits = Arrays.stream(tokens)
                .map(token -> token.toLowerCase(Locale.ROOT).replaceAll("[^\\p{L}]", ""))
                .filter(token -> !token.isBlank())
                .filter(CandidateNameExtractor::isSectionHeaderNoiseToken)
                .count();
        if (sectionHeaderNoiseHits > 0) {
            score -= 45.0;
            if (sectionHeaderNoiseHits >= tokens.length - 1) {
                score -= 40.0;
            }
        }
        if (looksLikeSectionHeaderNoiseLine(name)) {
            score -= 80.0;
        }
        return score;
    }
    ...
    private static boolean isSectionHeaderNoiseToken(String token) {
        if (token == null || token.isBlank()) {
            return false;
        }
        String normalized = normalizeAsciiLetters(token);
        if (normalized.isBlank()) {
            return false;
        }
        if (SECTION_HEADER_HINTS.contains(normalized)) {
            return true;
        }
        for (String prefix : SECTION_HEADER_PREFIX_HINTS) {
    ...
    private static boolean looksLikeSectionHeaderNoiseLine(String value) {
        if (value == null || value.isBlank()) {
            return false;
        }
        List<String> words = Arrays.stream(value.toLowerCase(Locale.ROOT).split("[^\\p{L}]+"))
                .map(CandidateNameExtractor::normalizeAsciiLetters)
                .filter(word -> !word.isBlank())
                .toList();
        if (words.isEmpty() || words.size() > 6) {
            return false;
        }
        long hintHits = words.stream().filter(CandidateNameExtractor::isSectionHeaderNoiseToken).count();
        if (hintHits >= Math.max(1, words.size() - 1)) {
            return true;
        }
        boolean hasAvailabilityStem = words.stream()
                .anyMatch(word -> word.contains("disponib") || word.contains("availab"));
        boolean hasImmediateStem = words.stream()
                .anyMatch(word -> word.contains("inmediat") || word.contains("immediat"));
        return hasAvailabilityStem && hasImmediateStem;
    }

    private static String normalizeAsciiLetters(String token) {
        if (token == null || token.isBlank()) {
            return "";
        }
        String folded = Normalizer.normalize(token, Normalizer.Form.NFD).replaceAll("\\p{M}+", "");
        return folded.toLowerCase(Locale.ROOT).replaceAll("[^a-z]", "");
    }
}
